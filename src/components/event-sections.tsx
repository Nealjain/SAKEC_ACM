    ...
    <>
      {events.map((event, index) => (
        ...
        >
          ...
          <div className={`relative z-10 max-w-7xl mx-auto px-4 w-full transition-all duration-1000 ${activeSection === index
          ...
          {activeSection === index && (
            <div className="absolute right-8 top-1/2 -translate-y-1/2 z-50 flex flex-col gap-3">
              {events.map((_, i) => (
                <div
                  key={i}
                  className={`w-2 h-2 rounded-full transition-all duration-500 ${i === index ? 'bg-white h-8' : 'bg-white/30'
                    }`}
                />
              ))}
            </div>
